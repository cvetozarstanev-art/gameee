# Christmas Memory Game (Console-hosted)

This repo contains a small Christmas-themed memory game (HTML/CSS/JS) served by a minimal .NET console HTTP server.
...
## Single-file bundle (copy/paste reference)

The project is made of multiple files, but the full source is included below in a single combined block so it can be pasted into a GitHub README.
...
Console.WriteLine("🎄 Starting Christmas Memory server...");
Console.WriteLine($"Open: http://localhost:{port}");
Console.WriteLine("Press Ctrl+C to stop.");

var server = new SimpleHttpServer(port, rootPath);
...
sealed class SimpleHttpServer
{
    ...
    public async Task StartAsync()
    {
        ...
        {
            Console.WriteLine($"Static root not found: {_rootPath}");
            Console.WriteLine("Make sure `wwwroot` is copied to the output directory.");
            return;
        }
    ...
    private async Task HandleRequestAsync(HttpListenerContext context)
    {
        try
        {
            ...
            var requestedPath = request.Url?.LocalPath ?? "/";
            ...
            var fullFile = Path.GetFullPath(combinedPath);

            if (!fullFile.StartsWith(fullRoot, StringComparison.OrdinalIgnoreCase))
            {
                response.StatusCode = 400;
                await WriteTextAsync(response, "Bad request");
                return;
            }

            if (File.Exists(fullFile))
            {
                var buffer = await File.ReadAllBytesAsync(fullFile);
                response.ContentType = GetContentType(fullFile);
            ...
            await WriteTextAsync(response, "404 - Not found");
        }
        catch (Exception ex)
        {
...
// =============================
// FILE: ConsoleApp1/wwwroot/index.html
// =============================
<!DOCTYPE html>
<html lang="en">
<head>
...
// =============================
// FILE: ConsoleApp1/wwwroot/style.css
// =============================
...
// =============================
// FILE: ConsoleApp1/wwwroot/app.js
// =============================
// --- Music (WebAudio) ---
class MusicPlayer {
